...
| 값 | 주인 | 개발이 읽을 자리 |
|----|------|----------------|
| **상태 필드 이름** · **단계 간 전달 집합(`K-n`)의 키 이름** | ③ 패턴/시퀀스 | ③ 「State 구조」 · 「단계 간 전달 집합」 |
| 단계ID · 타임아웃 · 재시도 · 반복 상한(`max_iter`) · 모델 호출 횟수 | ③ 패턴/시퀀스 | ③ 「단계별 설계」 |
| 담당자 수·묶음 · 저장소·도구의 **논리 이름과 권한 등급** · 승인자 · 사용 모델 | ④ 역할계약서 | ④ 「담당자별 계약」 7항목 |
| 표·열 · 커넥터 번호 · **커넥터 `외부규격` 키** · **메타데이터 필터 키** · 민감 필드 ID(`F-n`) | ⑤ 지식/도구 | ⑤ 3-1 · 3-2 · 3-3 |
| 비용 금액·상한 · 기록 항목 · 마스킹 방법 · 차단 규칙 | ⑥ 가드레일/관측 | ⑥ 3-1 ~ 3-3 |
| 배포 단위 · 포트 · 비밀값 주입 경로 · 저장소 배치 · **기술 스택 6행** | ⑦ 배포 | ⑦ 3-1 ~ 3-4 · 2-3 |

**④ 역할계약서는 키 이름을 하나도 갖지 않음** — 저장소·도구의 이름과 등급까지만 정함.
개발이 키 이름을 찾을 곳은 ③(우리쪽)과 ⑤(외부규격·필터)뿐임.
...
| 키 이름이 설계서와 다름 | 개발이 새 이름을 지었음 | ③ 「단계 간 전달 집합」·「State 구조」의 이름을 그대로 베낌(커넥터 외부 규격은 ⑤ 「입출력 키」). 새 이름 금지 |
